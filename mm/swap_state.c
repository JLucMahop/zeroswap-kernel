...
static bool enable_vma_readahead __read_mostly = true; //back in days was true
				    ...
				    struct mempolicy *mpol, pgoff_t ilx)
{
	...
	bool page_allocated;
	/*pr_info("swap_cluster_readahead: entry=%lx, offset=%lx, ilx=%lu\n",
		entry.val, offset, ilx);	*/
	mask = swapin_nr_pages(offset) - 1;
	/*pr_info("swap_cluster_readahead: entry=%lx, offset=%lx, ilx=%lu, mask=%lx\n",
		entry.val, offset, ilx, mask);*/
	if (!mask)
		goto skip;
	...
	for (offset = start_offset; offset <= end_offset ; offset++) {
		...
		pr_info("Offset: folio rsca \n");

		if (!folio)
			continue;
		if (page_allocated) {
			pr_info("swap_cluster_readahead: folio allocated\n");
...
skip:
	...
	/*pr_info("MAJOR FAULT: swap_cluster_readahead: entry=%lx, offset=%lx, ilx=%lu\n",
		entry.val, offset, ilx, mask);*/
	}
	return folio;
}

//handle sidecase when it is VM that is being swapped
struct folio *swap_cluster_readahead_dbs(swp_entry_t entry, gfp_t gfp_mask,
				    struct mempolicy *mpol, pgoff_t ilx,bool is_dbs)
{
	struct folio *folio;
	unsigned long entry_offset = swp_offset(entry);
	unsigned long offset = entry_offset;
		...
		struct mempolicy *mpol, pgoff_t targ_ilx, struct vm_fault *vmf)
{
	...
	pr_info("12: The window was win=%d, start=%lx, end=%lx\n",
		win, start, end);
	blk_start_plug(&plug);
	...
	lru_add_drain();
skip:
	pr_info("3: The window was win=%d, start=%lx, end=%lx\n",
		win, start, end);
...
struct folio *swapin_readahead_dbs(swp_entry_t entry, gfp_t gfp_mask,
				struct vm_fault *vmf,bool is_dbs)
{
	struct mempolicy *mpol;
	pgoff_t ilx;
	struct folio *folio;
	...
	folio = swap_use_vma_readahead() ?
		swap_vma_readahead(entry, gfp_mask, mpol, ilx, vmf) :
		swap_cluster_readahead_dbs(entry, gfp_mask, mpol, ilx,is_dbs);
	mpol_cond_put(mpol);
