		...
		struct writeback_control *wbc, struct swap_info_struct *sis)
{
	...
	unsigned long vaddr = xa_is_value(vaddr_entry)?xa_to_value(vaddr_entry):0;

	if (vaddr > 0) {
		unsigned long prev_addr = pages_va_map_get(va2sec_map, vaddr); //,true,true
		if(prev_addr > 0){
			pr_info("ALERTDBS SYNC VA OVERLAPPING %lx",vaddr);
		}
		pages_va_map_set(va2sec_map, vaddr, sector); //,true,true
		xa_erase(&dbs_pfn_map, folio_pfn(folio));
	}
	/* zeroswap */	
	submit_bio_wait(&bio);
	__end_swap_bio_write(&bio);
		...
		struct writeback_control *wbc, struct swap_info_struct *sis)
{
	...
	if (vaddr > 0) {
		//pages_va_map_set_folio(p2va_map, folio_pfn(folio), 0); //,false,false
		unsigned long prev_addr = pages_va_map_get(va2sec_map, vaddr); //,true,true
		if(prev_addr > 0){
			pr_info("ALERTDBS VA OVERLAPPING %lx",vaddr);
		}
		pages_va_map_set(va2sec_map, vaddr, sector); //,true,true
		//xa_store(&dbs_pfn_map, folio_pfn(folio), xa_mk_value(0), GFP_KERNEL);
		xa_erase(&dbs_pfn_map, folio_pfn(folio));
		//folio->private = 0;
	}else{
