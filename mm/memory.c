...
/*void print_event_kernel(unsigned long addr, unsigned long sech)
{
    struct timespec64 ts;
    ktime_get_raw_ts64(&ts);
    u64 now_ns = ts.tv_sec * 1000000000ULL + ts.tv_nsec;

    pr_info("KERNEL,%llu,%llu,%llu\n", addr, sech, now_ns);
}*/
...
static int get_process_name_using_vma(struct vm_area_struct *vma, char *name) {
    ...
    for_each_process(task) {
        if (task->mm == vma->vm_mm) {
           ...
           // snprintf(time, time_size, "%llu", task->start_time);
            return 0;
        }
    }
			...
			if ( is_fc){
				...
				if (is_dbs) {	
					/* Page has been "swapped by Guest".
					   ...
					   so the Guest will handle content reloading itself. */
					//void *shadow = NULL;
					...
					folio = folio_alloc_mpol(GFP_HIGHUSER_MOVABLE, 0, mpol, ilx, numa_node_id());
					...
					if (folio) {
						...
						//vmf->pte = __pte_offset_map(vmf->pte, vmf->pmd);
						if (likely(vmf->pte)) {
							pte_t pte = mk_pte(&folio->page, vma->vm_page_prot);
							set_pte_at(vma->vm_mm, vmf->address, vmf->pte, pte);
							//update_mmu_cache_range(vmf, vma, vmf->address, vmf->pte, 1);
						...
						ret = VM_FAULT_MAJOR;
						/*end_ns = ktime_get_ns();         // again in nanoseconds
...
check_folio:
	...
	if(ret == VM_FAULT_MAJOR){
		pr_info("NORMAL_ELAPSED,%llu\n", delta_ns);
	}else{
		pr_info("%d NORMAL_ELAPSED,%llu\n",ret, delta_ns);
	}*/
unlock:
	if (vmf->pte)
		pte_unmap_unlock(vmf->pte, vmf->ptl);
