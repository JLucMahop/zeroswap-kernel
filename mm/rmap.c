...
struct log_work_data {
 ...
 static int get_process_name_using_vma_1(struct vm_area_struct *vma, char *name) {
...
/*
void write_to_swapout(unsigned long addr)
{
    struct file *f;
    char buf[128];
    int len;
	u64 now_ns = ktime_get_real_ns();
    len = snprintf(buf, sizeof(buf), "%lu,%llu\n", addr, now_ns);
    f = filp_open("/mnt/sdb/test/out.txt", O_WRONLY | O_CREAT | O_APPEND, 0644);
    if (IS_ERR(f)) return;
			  ...
			  unsigned long address, void *arg)
 {
	...
	get_process_name_using_vma_1(vma,process_name);
	bool is_fc  = false;
	is_fc = strcmp(process_name,"firecracker") == 0;
	if(is_fc){
		...
		*/
		...
		xa_store(&dbs_pfn_map, folio_pfn(folio), xa_mk_value(address), GFP_KERNEL);		//folio->private = (void *)address; // Store the address in folio->private
	}
